...
template <typename Block, typename ReadPolicy, typename MemoryPolicy>
auto BlockInputStream<Block, ReadPolicy, MemoryPolicy>::Front() -> ValueType& {
...
template <typename Block, typename ReadPolicy, typename MemoryPolicy>
auto BlockInputStream<Block, ReadPolicy, MemoryPolicy>::FrontBlock() -> BlockPtr {
...
//      TRACEX(("block %014p => input queue (%d)") % BlockTraits<Block>::RawPtr(block) %
//             blocks_queue_.size());
      cv_.notify_one();
    }
  }
...
template <typename Block, typename ReadPolicy, typename MemoryPolicy>
auto BlockInputStream<Block, ReadPolicy, MemoryPolicy>::ReadBlock() -> BlockPtr {
...
//    TRACEX(("block %014p is empty, ignoring") % BlockTraits<Block>::RawPtr(block));
...
//    TRACEX(("block %014p <= input queue (%d)") % BlockTraits<Block>::RawPtr(block_) %
//           blocks_queue_.size());
  }
}
