...
template <typename Block> class BlockFileReadPolicy {
public:
  ...
  /// Set/get properties
  void set_input_filename(const std::string& ifn) {
    input_filename_ = ifn;
  }
  ...
  void set_input_rm_file(bool rm) {
    input_rm_file_ = rm;
  }
  bool input_rm_file() const {
    return input_rm_file_;
  }
...
template <typename Block> void BlockFileReadPolicy<Block>::Open() {
  //    TRACEX_METHOD();
  FileOpen();
}

template <typename Block> void BlockFileReadPolicy<Block>::Close() {
  //    TRACEX_METHOD();
  FileClose();
}

template <typename Block> void BlockFileReadPolicy<Block>::Read(BlockPtr& block) {
  FileRead(block);
  block_cnt_++;
}

template <typename Block> bool BlockFileReadPolicy<Block>::Empty() const {
...
template <typename Block> void BlockFileReadPolicy<Block>::FileOpen() {
  ...
  if (!ifs_) {
    std::cout << "Failed to open input file: " + input_filename_ << std::endl;
  }
}

template <typename Block> void BlockFileReadPolicy<Block>::FileRead(BlockPtr& block) {
...
template <typename Block> void BlockFileReadPolicy<Block>::FileClose() {
  if (ifs_.is_open()) {
    ifs_.close();
    if (input_rm_file_) {
      if (remove(input_filename_.c_str()) != 0) {
        std::cout << "Failed to remove file " << input_filename_ << std::endl;
      }
    }
  }
}
