...
template <typename SizeType> SizeType memsize_in_bytes(const SizeType& memsize, const MemUnit& u) {
  if (u == KB) {
    return memsize << 10;
  }
  if (u == MB) {
...
std::string
make_tmp_filename(const std::string& prefix, const std::string& suffix, const IndexType& index) {
  std::ostringstream filename;
  filename << prefix << "." << suffix << "." << std::setfill('0') << std::setw(3) << index;
  return filename.str();
}
...
sort_and_write(typename Types<ValueType>::BlockPtr block,
               ...
               typename Types<ValueType>::Comparator comparator) {
  // sort the block
  std::sort(block->begin(), block->end(), comparator);
  TRACE(("block %014p sorted") % Types<ValueType>::BlockTraits::RawPtr(block));
...
//! External Split
template <typename ValueType>
void split(SplitParams& params, typename Types<ValueType>::Comparator comparator) {
  TRACE_FUNC();
  size_t file_cnt = 0;
  ...
  auto mem_pool =
      std::make_shared<typename Types<ValueType>::BlockPool>(memsize_in_bytes(params.mem.size,
                                                                              params.mem.unit),
                                                             params.mem.blocks);
  ...
  while (!istream->Empty()) {
    ...
    ostream->set_output_filename(make_tmp_filename(params.spl.ofile, DEF_SPL_TMP_SFX, ++file_cnt));
    ostream->Open();

    // asynchronously sort the block and write it to the output stream
    splits.Async(&sort_and_write<ValueType>, std::move(block), std::move(ostream),
                 std::move(comparator));
...
//! External Merge
template <typename ValueType>
void merge(MergeParams& params, typename Types<ValueType>::Comparator comparator) {
  TRACE_FUNC();
  size_t file_cnt = 0;

  aux::AsyncFuncs<typename Types<ValueType>::OStreamPtr> merges;

  size_t mem_merge = memsize_in_bytes(params.mem.size, params.mem.unit) / params.mrg.merges;
  ...
  while (files.size() > 1 || !merges.Empty()) {
    ...
    ostream->set_output_filename(
        make_tmp_filename((params.mrg.tfile.size() ? params.mrg.tfile : params.mrg.ofile),
    ...
    merges.Async(&merge_streams<typename Types<ValueType>::IStreamPtr,
                                typename Types<ValueType>::OStreamPtr>,
                 std::move(istreams), std::move(ostream), std::move(comparator));
    ...
    while ((files.size() < params.mrg.kmerge && !merges.Empty()) || (merges.Ready() > 0) ||
           (merges.Running() >= params.mrg.merges)) {
  ...
  if (files.size()) {
    if (rename(files.front().c_str(), params.mrg.ofile.c_str()) == 0) {
      ...
      params.err.stream << "Cannot rename " << files.front() << " to " << params.mrg.ofile;
    }
  } else {
    params.err.none = false;
...
template <typename ValueType>
void sort(SplitParams& sp, MergeParams& mp, typename Types<ValueType>::Comparator comparator = {}) {
  split<ValueType>(sp, comparator);

  if (sp.err.none) {
    mp.mrg.ifiles = sp.out.ofiles;
    merge<ValueType>(mp, comparator);
  }
}

//! External Check
template <typename ValueType> bool check(CheckParams& params) {
  TRACE_FUNC();
  auto comp = typename ValueTraits<ValueType>::Comparator();
  auto vtos = typename ValueTraits<ValueType>::Value2Str();

  auto istream = std::make_shared<typename Types<ValueType>::IStream>();
  istream->set_mem_pool(memsize_in_bytes(params.mem.size, params.mem.unit), params.mem.blocks);
  ...
  if (!istream->Empty()) {
    ...
    while (!istream->Empty()) {
      ...
      if (comp(vcurr, vprev)) {
        if (bad < 10) {
          params.err.stream << "Out of order! cnt = " << cnt << " prev = " << vtos(vprev)
                            << " curr = " << vtos(vcurr) << "\n";
        }
        bad++;
      }
    ...
    params.err.stream << "\tmin = " << vtos(vmin) << ", max = " << vtos(vmax) << "\n";
    params.err.stream << "\tfirst = " << vtos(vfirst) << ", last = " << vtos(vprev) << "\n";
  }
  params.err.stream << "\tsorted = " << ((bad) ? "false" : "true") << ", elems = " << cnt
                    << ", bad = " << bad;
  istream->Close();
  return bad == 0;
}
...
//! External Generate
template <typename ValueType> void generate(const GenerateParams& params) {
  TRACE_FUNC();

  auto generator = typename ValueTraits<ValueType>::Generator();
  size_t gen_elements = memsize_in_bytes(params.gen.fsize, params.mem.unit) / sizeof(ValueType);

  auto ostream = std::make_shared<typename Types<ValueType>::OStream>();
  ostream->set_mem_pool(memsize_in_bytes(params.mem.size, params.mem.unit), params.mem.blocks);
