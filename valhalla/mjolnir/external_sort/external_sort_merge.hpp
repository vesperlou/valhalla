...
template <typename T> using StreamSet = std::unordered_set<T>;
...
void merge_2streams(StreamSet<InputStream*>& sin, OutputStream* sout, Comparator comp) {
  //    TRACE_FUNC();
  if (sin.size() != 2) {
//    LOG_ERR(("Internal error: mismatch in number of streams %d/%d") % sin.size() % 2);
    return;
  }
  auto it = sin.begin();
  InputStream* s1 = *(it++);
...
// merges 3 streams
template <typename InputStream, typename OutputStream, typename Comparator>
void merge_3streams(StreamSet<InputStream*>& sin, OutputStream* sout, Comparator comp) {
  //    TRACE_FUNC();
  if (sin.size() != 3) {
//    LOG_ERR(("Internal error: mismatch in number of streams %d/%d") % sin.size() % 3);
    return;
  }
  auto it = sin.begin();
  InputStream* s1 = *(it++);
...
// merges 4 streams
template <typename InputStream, typename OutputStream, typename Comparator>
void merge_4streams(StreamSet<InputStream*>& sin, OutputStream* sout, Comparator comp) {
  //    TRACE_FUNC();
  if (sin.size() != 4) {
//    LOG_ERR(("Internal error: mismatch in number of streams %d/%d") % sin.size() % 4);
    return;
  }
  auto it = sin.begin();
  InputStream* s1 = *(it++);
...
template <typename InputStream, typename OutputStream, typename Comparator>
void merge_nstreams(StreamSet<InputStream*>& sin, OutputStream* sout, Comparator comp) {
  //    TRACE_FUNC();
  if (sin.size() <= 4) {
//    LOG_ERR(("Internal error: too few streams for heap-based merge %d") % sin.size());
    return;
  }
  ...
  auto hcomp = [&comp](InputStream*& s1, InputStream*& s2) { return comp(s2->Front(), s1->Front()); };
...
OutputStreamPtr merge_streams(StreamSet<InputStreamPtr> sin,
                              ...
                              typename Types<typename InputStreamPtr::element_type::BlockType::value_type>::Comparator comp = {}) {
  //    TRACE_FUNC();
...
//  auto comp = typename Types<typename InputStream::BlockType::value_type>::Comparator();
  ...
  if (sinp.size() > 0) {
    ...
    std::cout << "No input streams to merge!\n";
    sout.reset();
  }
