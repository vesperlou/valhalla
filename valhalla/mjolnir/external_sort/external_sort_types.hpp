...
//! Default generator
template <typename T> struct DefaultValueGenerator {
  T operator()() {
    union {
      T data;
...
template <typename ValueType> struct DefaultValue2Str {
...
template <typename ValueType> struct ValueTraits {
...
template <typename ValueType> struct Types {
  ...
  // Stream Types
  using IStream = block::
      BlockInputStream<Block, block::BlockFileReadPolicy<Block>, block::BlockMemoryPolicy<Block>>;

  using OStream = block::
      BlockOutputStream<Block, block::BlockFileWritePolicy<Block>, block::BlockMemoryPolicy<Block>>;
