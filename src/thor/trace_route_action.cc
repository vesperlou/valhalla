  ...
  for (const auto& result : topk_match_results) {
    ...
    for (const auto& path : pathes) {
      for (const auto& ele : path) {
        path_edges.push_back(ele.first);
      }
    }
...
////////////////////////////////////////////////////////////////////////////
    ...
    if (options.action() == Options::trace_attributes) {
      ...
      serilize_pathes(pathes, match_results, route_discontinuities, options, request);
      //      serilize_pathes_old(path_edges, match_results, disconnected_edges,
      //      route_discontinuities,
      //                          options, request);
    }
...
void thor_worker_t::serilize_pathes(
  ...
  const PathInfo* last_edge{&pathes.front().front().first};
  ...
  for (const auto& path : pathes) {
    ...
    auto first_iter = std::find_if(path.cbegin(), path.cend(),
                                   [](const auto& ele) { return ele.second->first_match_idx >= 0; });
    auto last_iter = std::next(std::find_if(path.rbegin(), path.rend(), [](const auto& ele) {
                       return ele.second->last_match_idx >= 0;
                     })).base();

    auto first_segment = first_iter->second;
    auto last_segment = last_iter->second;

    auto origin_iter = first_iter;
      ...
      for (int dest_match_idx = origin_match_idx + 1; dest_match_idx <= last_match_idx;
        ...
        auto dest_iter =
            std::find_if(origin_iter, last_iter + 1, [&dest_match, dest_match_idx](const auto& ele) {
              return dest_match.edgeid == ele.second->edgeid &&
                     dest_match.distance_along >= ele.second->source &&
                     dest_match.distance_along <= ele.second->target;
            });
        ...
        auto leg_begin = edges.cbegin();
        auto leg_end = edges.cend() - 1;

        // if its the first or last path info in the edge group then its been already trimmed
        bool begin_trimmed = origin_match.edgeid == first_segment->edgeid &&
                             origin_iter->second->source == first_segment->source &&
                             origin_iter->second->target == first_segment->target;
        bool end_trimmed = dest_match.edgeid == last_segment->edgeid &&
                           dest_iter->second->source == last_segment->source &&
                           dest_iter->second->target == last_segment->target;
        bool trivial_group = first_iter == last_iter;
        ...
        double begin_pct =
            begin_trimmed ? match_results[first_iter->second->first_match_idx].distance_along : 0;
        double end_pct =
            end_trimmed ? match_results[last_iter->second->last_match_idx].distance_along : 1;
        ...
        double trim_begin = origin_iter == pathes.cbegin()->cbegin()
                                ? 0.0
                                : last_edge->elapsed_time + leg_begin->turn_cost;
        ...
        trim_begin += (leg_begin->elapsed_time - trim_begin) *
                      (origin_match.distance_along - begin_pct) * begin_edge_scale;

        std::cout << leg_begin->elapsed_time << " " << origin_match.distance_along << " " << begin_pct
                  << " " << begin_edge_scale << std::endl;
        ...
        double trim_end;
        if (dest_iter == pathes.cbegin()->cbegin()) {
          trim_end = 0.0;
        } else if (trivial_group) {
          trim_end = last_edge->elapsed_time + leg_end->turn_cost;
        } else {
          trim_end = std::prev(leg_end)->elapsed_time + leg_end->turn_cost;
        }
        std::cout << "trim_end before " << trim_end << std::endl;
        //
        //
        //        double trim_end = dest_iter == pathes.cbegin()->cbegin()
        //                              ? 0.0
        //                              : std::prev(leg_end)->elapsed_time + leg_end->turn_cost;

        // then we scale the elapsed time on the edge based on the distance along the edge but
        // we need to scale that distance down in the case FormPath knew it was a partial edge
        trim_end = (leg_end->elapsed_time - trim_end) * (end_pct - dest_match.distance_along) *
                   end_edge_scale;
        std::cout << "trim_end after " << trim_end << std::endl;

        std::cout << " trim begin " << trim_begin << ", trim end " << trim_end << std::endl;
        std::cout << "build legs on " << origin_match_idx << " " << dest_match_idx << std::endl;
        ...
        // last_edge = &dest_iter->first;
      }
      last_edge = &path.back().first;
    }
  }
}

void thor_worker_t::serilize_pathes_old(
